...
--USE library SELECT DISTINCT member_no, isbn, fine_assessed, fine_assessed * 2 AS 'double fine' FROM loanhist WHERE ISNULL(fine_assessed, 0) > 0
--USE library SELECT REPLACE(LOWER(firstname) + LOWER(middleinitial) + LOWER(SUBSTRING(lastname, 1, 2)), ' ', '') AS 'email_name' FROM member WHERE lastname = 'Anderson'
