...
---USE Northwind SELECT EmployeeID, Year(GETDATE()) - Year(HireDate) FROM Employees

---Policz sumę lat przepracowaną przez wszystkich pracowników i średni czas w firmie
---USE Northwind SELECT SUM(Year(GETDATE()) - Year(HireDate)), AVG(Year(GETDATE()) - Year(HireDate)) FROM Employees
